...
public class PlayerSkinHandler implements PacketHandler<PlayerSkinPacket> {
    ...
    public void handle(PlayerSkinPacket packet, PlayerSessionHolder holder, Server server) {
        ...
        // Check the cooldown before converting and validating the skin, so spamming this packet costs nothing
        long cooldown = TimeUnit.SECONDS.toMillis(player.getServer().getSettings().playerSettings().skinChangeCooldown());
        if (System.currentTimeMillis() - player.lastSkinChange < cooldown){
